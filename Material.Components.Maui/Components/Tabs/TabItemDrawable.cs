...
internal class TabItemDrawable
{
    ...
    {
        if (!this.view.HasIcon || this.view.Image != null || this.view.Icon is IconKind.None) return;
        canvas.Save();
    ...
    private void DrawText(SKCanvas canvas, SKRect bounds)
    {
        ...
        var y = this.view.HasIcon ? 36 + ((25 - this.view.TextBlock.MeasuredHeight) / 2) : bounds.MidY - (this.view.TextBlock.MeasuredHeight / 2);
    ...
    private void DrawActiveIndicator(SKCanvas canvas, SKRect bounds)
    {
        ...
        rect.SetRectRadii(new SKRect(x, y, x + 40, y + 3), this.view.ActiveIndicatorShape.GetRadii());
    ...
    private void DrawRippleEffect(SKCanvas canvas, SKRect bounds)
    {
        ...
        canvas.DrawRippleEffect(bounds, 0, this.view.RippleSize, point, color, this.view.RipplePercent);
    }
}
